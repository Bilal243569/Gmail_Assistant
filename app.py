...
def extract_and_respond(email_data):
    ...
    model = ChatGoogleGenerativeAI(model="gemini-2.0-flash-exp", api_key="AIzaSyC9jLpI6udKlA1LE424yqVxL58uv404AKU")
    extraction_prompt = f"""
    ...
    if "Sender" in details and "Content" in details and "Subject" in details:
        response_prompt = f"""
        Write a polite and professional response to the following email also add my name : muhammad bilal , also select a creative subject according to the response that u have created and  your response should not include this line = (Okay, here's a polite and professional response you can use,incorporating your name:)  and i am a receiver so subject should me according to the response:
...
def send_email_response(service, sender_email, subject, response_content):
    ...
    message["From"] = "bilal243569@gmail.com"  # Replace with your email
